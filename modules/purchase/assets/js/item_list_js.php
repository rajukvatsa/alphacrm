...
function edit_commodity_item(invoker){
  ...
  $('#commodity_list-add-edit').modal('show');
      ...
      $('#commodity_list-add-edit input[name="stock_in"]').val($(invoker).data('stock_in'));
  ...
  function new_commodity_item(){
    ...
    $.post(admin_url + 'purchase/get_commodity_barcode').done(function(response) {
      response = JSON.parse(response);
		...
		$('#commodity_list-add-edit input[name="sku_code"]').val(response);
		$('#commodity_list-add-edit input[name="sku_name"]').val(response);
