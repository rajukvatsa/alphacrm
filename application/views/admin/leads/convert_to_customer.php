...
<div class="modal fade" id="convert_lead_to_client_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-lg" role="document">
        <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBw9z34jxxsICyUDTjLQTTVnjFHJUahLVQ&libraries=places"></script>
              ...
              if (!in_array($c_field, $not_mergable_contact_fields)) {
                ...
                <div class="client_password_set_wrapper" style="display:none;">
                    ...
                    <div class="input-group">
                        <input type="password" class="form-control password" value="123456" name="password" autocomplete="off">
...
<script>
 $(document).on("shown.bs.modal", "#convert_lead_to_client_modal", function () {
    // Check if the modal contains the desired input field
    ...
    if (input) {
        ...
        autocomplete.addListener("place_changed", () => {
            ...
            // Parse the place address components
            place.address_components.forEach((component) => {
                ...
                if (types.includes("locality")) {
                    ...
                    country = component.short_name; // Use short_name for ISO country codes
            ...
            // Update the country dropdown
             let countryName = ""; // To store the country long_name

            // Extract country from the place address components
            if (place.address_components) {
                place.address_components.forEach((component) => {
                    if (component.types.includes("country")) {
                        countryName = component.long_name; // Use the full country name
                    }
                });
            }
            ...
            if (countryDropdown && countryName) {
                const options = countryDropdown.options;

                for (let i = 0; i < options.length; i++) {
                    // Match the dropdown option's text with the country name
                    if (options[i].textContent.trim() === countryName.trim()) {
                        countryDropdown.value = options[i].value; // Set the value to match the database ID
                        break;
                    }
                }

                // Refresh the selectpicker to reflect changes (if using Bootstrap)
