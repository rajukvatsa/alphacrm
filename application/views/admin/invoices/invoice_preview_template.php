...
<?php } ?>
<?php 
$delvveryOptions=array('Order sent to Warehouse','Ready For Shipment','Out For Delivery','Delivered');
//get delivery status 
$sTable       = db_prefix().'invoicepaymentrecords';  
$sql ="SELECT delivery_status FROM $sTable WHERE invoiceid =".$invoice->id;
$query = $this->db->query($sql);
$deliveryStatus="";
if ($query->num_rows() > 0) {
	$deliveryStatusRes=$query->result();
	
	$deliveryStatusRes=$deliveryStatusRes[0]->delivery_status;
}
...
<div class="col-md-12 no-padding">
                ...
                <div class="horizontal-tabs">
                    <ul class="nav nav-tabs nav-tabs-horizontal mbot15" role="tablist">
                        ...
                        <li role="presentation">
						 ...
						 <li role="presentation">
                            <a href="#tab_otracking"
                                onclick="#"
                                aria-controls="tab_otracking" role="tab" data-toggle="tab">
                                <?php echo _l('Order Tracking'); ?>
                            </a>
    ...
    [
            ...
            <div class="row mtop20">
                <div class="col-md-5">
			     ...
			     <?php if($deliveryStatusRes!="" ){ if($deliveryStatusRes=="pending"){$delClass="label-danger";}else{$delClass="label-info";}  ?>
					<span class="label <?php echo  $delClass; ?> mtop5 inline-block s-status invoice-status-2">Delivery Status: <?php echo  $deliveryStatusRes; ?></span>
                 <?php } ?>
			   </div>
                <div class="col-md-7 _buttons">
            ...
            <div class="tab-content">
				...
				<div role="tabpanel" class="tab-pane" id="tab_otracking">
				    <div class="clearfix"></div>
	...
	<div class="col-md-7">
	<?php echo form_open(admin_url('invoices/mark_as_delivered/' . $invoice->id), ['id' => 'sales-notes-delivery', 'class' => 'invoice-notes-form']); ?>
	<div class="dataTables_length" id="related_tasks_length">
	 
                  
	<label>
	<select name="delivery_status" aria-controls="related_tasks" class="form-control input-sm">
	<?php 
	$index= array_search($deliveryStatusRes,$delvveryOptions,true);
	array_splice($delvveryOptions, 0, $index);
	foreach($delvveryOptions as $delOpt){   ?>
	<?php if($deliveryStatusRes==$delOpt) { ?>
	<option selected  value="<?php echo  $delOpt; ?>"><?php echo  $delOpt; ?></option> 
	<?php } else{ ?>
	<option value="<?php echo  $delOpt; ?>"><?php echo  $delOpt; ?></option> 
	<?php } ?>
	<?php } ?>
	</select></label>
	</div>
	<div class="dt-buttons btn-group"> 
	         <button type="submit" class="mleft10  btn btn-success" tabindex="0" aria-controls="related_tasks"
			 aria-haspopup="true" aria-expanded="false"><span>Update Delivery Status</span></button> 
    ...
    <hr />
	<a href="#" data-toggle="modal" class="mleft10 pull-right btn btn-success" data-target="#sales_attach_file"><?php echo _l('Upload Reciept'); ?></a>
