            ...
            <div class="col-md-12">
                ...
                <div class="_buttons tw-mb-2 sm:tw-mb-4 tw-inline-block tw-w-full">
                    ...
                    <a href="#" class="btn btn-default pull-left mleft5" data-toggle="modal" data-target="#groups">
					  ...
					  <a href="#" class="btn btn-default pull-left mleft5" data-toggle="modal" data-target="#warehouse">
                        <i class="fa-solid fa-truck tw-mr-1"></i>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <?php if (staff_can('create',  'items')) { ?>
                <div class="input-group">
                    <input type="text" name="item_warehouse_name" id="item_warehouse_name" class="form-control"
                        placeholder="<?php echo _l('Warehouse name'); ?>">
                    <span class="input-group-btn">
                        <button class="btn btn-primary" type="button" id="new-item-warehouse-insert">
                            <?php echo _l('New Warehouse'); ?>
                        </button>
                    </span>
                </div>
                <hr />
                <?php } ?>
                    ...
                    //get warehouse stock 
                    $CI =& get_instance(); 
                    $query = $CI->db->query("SELECT sum(`unit`) as total,`warehouse_id` FROM `tblitems` WHERE 1 group by `warehouse_id`")->result_object();
                    $wQty=array();
                    if(!empty($query)){
                    	foreach($query as $warehouses){
                    		$wQty[$warehouses->warehouse_id]=$warehouses->total;
                    	}
                    }
                    ...
                    <div class="container-fluid">
                        <table class="table dt-table table-items-groups" data-order-col="1" data-order-type="asc">
                            <thead>
                                <tr>
                                    <th><?php echo _l('id'); ?></th>
                                    <th><?php echo _l('Warehouse Name'); ?></th>
                                    <th><?php echo _l('Stock Available'); ?></th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($items_warehouse as $group) { ?>
                                <tr class="row-has-options" data-group-row-id="<?php echo e($group['id']); ?>">
                                    <td data-order="<?php echo e($group['id']); ?>"><?php echo e($group['id']); ?></td>
                                    <td data-order="<?php echo e($group['name']); ?>">
                                        <span class="group_name_plain_text"><?php echo e($group['name']); ?></span>
...
$(function() {
  ...
  if (get_url_param('groups_warehouse')) {
        // Set time out user to see the message
        setTimeout(function() {
	...
	$('#new-item-warehouse-insert').on('click', function() {
        var group_name = $('#item_warehouse_name').val();
        if (group_name != '') {
            $.post(admin_url + 'invoice_items/add_warehouse', {
                name: group_name
            }).done(function() {
                window.location.href = admin_url + 'invoice_items?groups_warehouse=true';
            });
        }
    });
