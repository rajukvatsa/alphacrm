            ...
            if (($invoice->status != Invoices_model::STATUS_PAID
               ...
               && $invoice->total > 0)) { ?>
				...
				<div class="col-md-6 text-left">
				          <p class="tw-mb-2.5 tw-font-medium">Want Finance option ?</p>
							<div class= form-check form-check-inline">
							
                                <span><input type="radio" value="no"
                                    id="pm_fin_no" name="finmode">
                                <label for="pm_fin_no">No</label></span>
								<span><input type="radio" value="yes"
                                    id="pm_fin_yes" name="finmode">
                                <label for="pm_fin_yes">yes</label></span>
                            </div>
							
				</div>
				<div class="col-md-6 text-right">
			       <div  style="display:none;" id="finoptions">
				   <?php
				   $totalamount=$invoice->total_left_to_pay;
				   $depositamount=ceil($invoice->total_left_to_pay*30/100);
				   $finance=$invoice->total_left_to_pay-$depositamount;
				   ?>
                    <p><span>Deposit Amount 30%:  <?php echo e($invoice->symbol); ?><span id="deposit"><?php echo  number_format($depositamount,2) ?><span></span></p>
					<p><span >Finance Amount 70%:  <?php echo e($invoice->symbol); ?><span id="finance"><?php echo  number_format($finance,2) ?></span></span></p>
					<p><span><?php echo e($invoice->symbol); ?><span id="totalamount"><?php echo  $totalamount ?><span></span></p>
					</div>
                        ...
                        <div class="col-md-6 text-left">
                            ...
                            <div class="form-group mtop25">
                                ...
                                <div class="input-group">
                                    <input type="number" id="invoiceamount" required max="<?php echo e($invoice->total_left_to_pay); ?>"
...
$(function() {
    ...
    if (online_payments.length == 1) {
	 ...
	 $('#pm_fin_yes').on('click', function(e) {
            e.preventDefault();
           $('#finoptions').show();
		   
		   $('#invoiceamount').val($('#deposit').text());
		   
     });
	  $('#pm_fin_no').on('click', function(e) {
            e.preventDefault();
           $('#finoptions').hide();
		   $('#invoiceamount').val($('#totalamount').text());
