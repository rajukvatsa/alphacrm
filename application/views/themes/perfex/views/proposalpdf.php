...
// Add bank details from payment modes
$CI = &get_instance();
$CI->load->model('payment_modes_model');
$payment_modes_data = $CI->payment_modes_model->get('', [], false);
$bank_details = '';
if (!empty($payment_modes_data)) {
    foreach ($payment_modes_data as $mode) {
        if (isset($mode['show_on_pdf']) && $mode['show_on_pdf'] == 1 && !empty($mode['description'])) {
            $bank_details .= '<strong>' . $mode['name'] . ':</strong><br />' . $mode['description'] . '<br /><br />';
        }
    }
}
...
if (!empty($bank_details)) {
    $html .= '<br /><br /><div style="border-top:1px solid #ddd; padding-top:10px;"><strong>Payment Details:</strong><br /><br />' . $bank_details . '</div>';
}
