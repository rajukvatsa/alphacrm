   ...
   public function mark_as_delivered($id)
    {
        ...
        if (!user_can_view_invoice($id)) {
            access_denied('Order stauts delivery.');
        }

        $deliveryStatus="";
		if($this->input->post()){
			$postData=$this->input->post();
			$deliveryStatus=$postData['delivery_status'];
			$this->db->update(db_prefix().'invoicepaymentrecords', ['delivery_status' => $postData['delivery_status']], ['invoiceid'=>$id]);
		}
        ...
        if ($success) {
            set_alert('success', _l('Order delivery status changed to '.$deliveryStatus));
        } else {
            set_alert('warning', _l('Status update failed'));
        }
