...
class Payments_model extends App_Model
{
    ...
    public function add($data, $subscription = false)
    {
        ...
        // Handle haulage data
        if (isset($data['haulage_type'])) {
            $data['haulage_type'] = $data['haulage_type'];
        }
        if (isset($data['haulage_cost'])) {
            $data['haulage_cost'] = $data['haulage_cost'];
        }
    ...
    private function get_contacts_for_payment_emails($client_id)
    {
     ...
     public function update_stock_upon_payment($data)
    {
        // Assuming $data contains 'invoice_id' and 'payment_amount'
        ...
        foreach ($data as $item) {
            ...
            //get the item id from the invoice items table
              if(isset($item['description']) && $item['description'] !=''){
                 $item_parts = explode('_', $item['description']);
                if(count($item_parts) >=1){
                     $item['itemid'] = $item_parts[0];
                }
                    ...
                    $this->db->where('id', $item['itemid']);
                      ...
                      $current_stock = $this->db->get(db_prefix() . 'items')->row()->stock_in;
              ...
              if($current_stock){
                    ...
                    $this->db->where('id', $item['itemid']);
                    $this->db->update(db_prefix() . 'items', ['stock_in' => $new_stock]);
