...
class Invoice_items_model extends App_Model
{
    ...
    {
        $this->db->insert(db_prefix() . 'items_warehouse', $data);
        log_activity('Items Warehouse Created [Name: ' . $data['name'] . ']');
    ...
    public function get_warehouse()
    {
        ...
        return $this->db->get(db_prefix() . 'items_warehouse')->result_array();
    }
    public function edit_warehouse($data, $id)
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'items_warehouse', $data);
        if ($this->db->affected_rows() > 0) {
            log_activity('Items Warehouse Updated [Name: ' . $data['name'] . ']');
    ...
    {
        $this->db->where('id', $id);
        $group = $this->db->get(db_prefix() . 'items_warehouse')->row();
        ...
        if ($group) {
            ...
            $this->db->where('id', $id);
            $this->db->delete(db_prefix() . 'items_warehouse');

            log_activity('Item Warehouse Deleted [Name: ' . $group->name . ']');
